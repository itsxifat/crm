        ...
        <div className="flex flex-col gap-5 max-w-[1600px] mx-auto w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 self-start sm:self-auto">
              <button 
                ...
                className="inline-flex items-center gap-2 px-3 py-2 rounded-md border border-slate-200 bg-white text-xs font-bold text-slate-600 uppercase tracking-wide hover:bg-slate-50 hover:text-slate-900 transition-colors"
              >
              ...
              <button 
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-[#10a37f] text-xs font-bold text-white uppercase tracking-wide hover:bg-[#0d8a6a] transition-all shadow-sm shadow-emerald-500/20"
              >
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button 
              ...
              className={`relative inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border text-sm font-medium transition-all
                ${showFilters || activeFilterCount > 0
                  ? "bg-[#10a37f]/5 border-[#10a37f]/20 text-[#10a37f]" 
                  : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300"}`}
            >
              <Filter className="h-4 w-4" />
              <span>Filter</span>
              {activeFilterCount > 0 && (
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#10a37f] text-[10px] font-bold text-white shadow-sm">
                  {activeFilterCount}
                </span>
              )}
            </button>
            ...
            <div className="p-5 bg-white rounded-xl border border-slate-200 shadow-lg animate-in slide-in-from-top-2 duration-200">
              <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-3">
                ...
                <button 
                  ...
                  className="text-xs text-red-500 hover:text-red-700 font-bold uppercase tracking-wide hover:underline flex items-center gap-1"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {meta.services.length === 0 && <span className="text-xs text-slate-400 italic">None available</span>}
                    {meta.services.map(s => (
                      <button
                        key={s}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {meta.categories.length === 0 && <span className="text-xs text-slate-400 italic">None available</span>}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {meta.sources.length === 0 && <span className="text-xs text-slate-400 italic">None available</span>}
                    {meta.sources.map(s => (
                      <button
                        key={s}
                        onClick={() => toggleFilter(selectedSources, setSelectedSources, s)}
                        className={`px-2.5 py-1 rounded text-[11px] font-semibold border transition-all ${
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {meta.platforms.length === 0 && <span className="text-xs text-slate-400 italic">None available</span>}
        ...
        {!loading && filtered.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-6">
              ...
              <button 
                onClick={() => { setEditing(null); setShowModal(true); }}
                className="px-4 py-2 bg-[#10a37f] rounded-md text-sm font-medium text-white hover:bg-[#0d8a6a] shadow-sm transition-colors"
              >
                Create Lead
              </button>
            </div>
