...
function StatusPill({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold ring-1 ${cls}`}
    >
...
function AvatarBadge({ name }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-2 py-1 text-xs shadow-sm">
      <span
        className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-white ${palette[idx]}`}
      >
        {initials}
      </span>
...
export default async function ProjectDetailsPage(props) {
  ...
  // Absolute URLs (headers() is awaited inside absoluteUrl)
  ...
  // Services may or may not be present depending on your GET route; render defensively.
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(1000px_600px_at_10%_-10%,rgba(16,185,129,0.08),transparent),radial-gradient(800px_500px_at_90%_-20%,rgba(59,130,246,0.08),transparent)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        {/* Top header bar (shadow softened only) */}
        <div className="mb-8 overflow-hidden rounded-3xl border border-gray-200/60 bg-white shadow-[0_12px_32px_-24px_rgba(0,0,0,0.25)]">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-sm">
                ...
                <div className="font-medium text-gray-900">
                  {project.startDate || "—"} <span className="text-gray-400">→</span>{" "}
                  {project.dueDate || "—"}
                </div>
              </div>
            </div>
        ...
        <section id="print-area" className="grid gap-8 md:grid-cols-2">
                ...
                <div className="flex-1">
                  ...
                  {assignedUserNames.length ? (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {assignedUserNames.map((n) => (
                        <AvatarBadge key={n} name={n} />
                      ))}
                    </div>
                  ) : (
          ...
          {/* Financials (shadow softened only) */}
          <div className="rounded-3xl border border-gray-200/70 bg-white/90 p-6 shadow-[0_6px_18px_-12px_rgba(0,0,0,0.2)] backdrop-blur">
            <h2 className="text-lg font-semibold tracking-tight text-gray-900">
              Financials
            </h2>
          ...
          {/* Summary card: now also shows Services (no layout changes; shadow softened) */}
          <div className="md:col-span-2 rounded-3xl border border-gray-200/70 bg-white/90 p-6 shadow-[0_6px_18px_-12px_rgba(0,0,0,0.2)] backdrop-blur">
            ...
            {/* Services List (renders only if available) */}
                ...
                <table className="min-w-full text-sm">
                  ...
                  <tfoot>
                    <tr className="border-t border-gray-100">
                      <td className="px-3 py-3 text-right font-medium text-gray-600" colSpan={3}>
                        Subtotal
                      </td>
            ...
            {/* Keep your original explanatory paragraph */}
            <p className="mt-6 text-sm leading-relaxed text-gray-600">
              This executive report consolidates project metadata, schedule, and
              financials in a clean, exportable format. Use{" "}
              <strong>Export PDF</strong> to generate a client-ready report.
            </p>
          </div>
        </section>
      </div>
