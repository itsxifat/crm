...
function safeDateISO(dLike) { return String(dLike || new Date().toISOString().slice(0, 10)).slice(0, 10); }
...
async function buildClientProjects(db, clientId) {
  ...
  const projFilter = {
    $or: [
      ...
      ...(ObjectId.isValid(clientKeyStr) ? [{ clientId: new ObjectId(clientKeyStr) }, { client: new ObjectId(clientKeyStr) }] : []),
    ],
  };
...
export async function PATCH(req, ctx) {
  try {
    ...
    // ---- Determine addAmount correctly (key fix) ----
    // For client invoices + allocation, "full" means sum of selected projects' due, not whole client due.
    let addAmount = round2(amount || 0);

    if (String(inv.source || "").toLowerCase() !== "client") {
      // project invoice -> allow markFullPaid to fill full due
      ...
      if (markFullPaid && allocation) {
        ...
        if (allocation.mode === "single" && allocation.projectIds?.length) {
          ...
          addAmount = round2(allocation.projectIds.reduce((s, pid) => s + Number(byId.get(String(pid)) || 0), 0));
    ...
    const newPayments = [...prevPayments, { dateISO: safeDate, amount: addAmount, note: note || "" }];
    ...
    if (String(inv.source || "").toLowerCase() === "client" && allocation) {
      ...
      const remainingDue = (c) => Math.max(0, round2(Number(c.total || 0) - Number(c.paidAmount || 0)));
      ...
      if (allocation.mode === "single" && allocation.projectIds?.length) {
        ...
        if (target) {
          const take = Math.min(addAmount, remainingDue(target)); // cap to that project's remaining due
        ...
        if (allocation.splitEvenly) {
          // split evenly with caps, then spill any leftover by capacity
          ...
          // proportional by remaining due with caps and spill
    ...
    // 4) regenerate current invoice PDF (with fresh clientProjects if parent)
